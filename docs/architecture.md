...
Nice3point.Revit.Toolkit exists to remove the boilerplate of building Revit add-ins. It wraps the awkward parts of the Revit API — `IExternalCommand`/`IExternalApplication` plumbing, external events, the threading model, assembly resolution, and option callbacks — behind clean, discoverable, production-grade types.
...
The `[ExternalEvent]` source generator removes the per-handler boilerplate: decorate a method on a `partial` type and the generator emits the matching `IExternalEvent`/`IAsyncExternalEvent` property, choosing sync vs. request/response and parameterized vs. argument-record forms from the method signature. The analyzers enforce the prerequisites (partial type, non-async-void, non-generic, no overloads). See [Analyzers & Source Generators](./analyzers-and-generators.md).
...
* Wrap the Revit API; do not reimplement it. Toolkit types add ergonomics and lifecycle management, not new behavior.

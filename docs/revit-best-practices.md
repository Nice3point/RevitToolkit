...
* Suppress UI during programmatic operations with `RevitContext.BeginDialogSuppressionScope()` / `RevitApiContext.BeginFailureSuppressionScope()`, and always restore via the disposable scope.
...
* Use `#if NET` / `#if NET8_0_OR_GREATER` for runtime differences (e.g. `AssemblyLoadContext`-aware assembly resolution in `ExternalCommand`).
